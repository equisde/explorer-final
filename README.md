...
##### Compile latest SevaBit development version
...
When compilation finishes executable `SevaBitBlocks` should be created. Before running
...
```
SevaBitBlocks, Onion SevaBit Blockchain Explorer:
...
alias SevaBitBlocksmainnet='~/onion-SevaBit-blockchain-explorer/build/SevaBitBlocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias SevaBitBlockstestnet='~/onion-SevaBit-blockchain-explorer/build/SevaBitBlocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```
...
```bash
SevaBitBlocks --enable-emission-monitor 
```
...
Having the `crt` and `key` files, run `SevaBitBlocks` in the following way:

```bash
./SevaBitBlocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key 
```
