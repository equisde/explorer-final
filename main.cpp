...
main(int ac, const char* av[])
{
    ...
    if (testnet && daemon_url == "http:://127.0.0.1:22049")
        daemon_url = "http:://127.0.0.1:22049";
    if (stagenet && daemon_url == "http:://127.0.0.1:22049")
        daemon_url = "http:://127.0.0.1:22049";
    ...
    lokeg::page SevaBitBlocks(&mcore,
                          core_storage,
                          daemon_url,
                          nettype,
                          enable_pusher,
    ...
    ([&](const crow::request& req) {
        return crow::response(SevaBitBlocks.index2());
    });

    CROW_ROUTE(app, "/page/<uint>")
    ([&](size_t page_no) {
        return SevaBitBlocks.index2(page_no);
    });

    CROW_ROUTE(app, "/block/<uint>")
    ([&](const crow::request& req, size_t block_height) {
        return crow::response(SevaBitBlocks.show_block(block_height));
    });

    CROW_ROUTE(app, "/block/<string>")
    ([&](const crow::request& req, string block_hash) {
        return crow::response(SevaBitBlocks.show_block(remove_bad_chars(block_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>")
    ([&](const crow::request& req, string tx_hash) {
        return crow::response(SevaBitBlocks.show_tx(remove_bad_chars(tx_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>/<uint>")
    ([&](string tx_hash, uint16_t with_ring_signatures)
     {
        return SevaBitBlocks.show_tx(remove_bad_chars(tx_hash), with_ring_signatures);
    });

    CROW_ROUTE(app, "/service_node/<string>")
    ([&](const crow::request& req, std::string service_node_pubkey) {
        return crow::response(SevaBitBlocks.show_service_node(remove_bad_chars(service_node_pubkey)));
    });

    CROW_ROUTE(app, "/service_nodes")
    ([&](const crow::request& req) {
        return SevaBitBlocks.render_service_nodes_html(true /*add_header_and_footer*/);
    });

    CROW_ROUTE(app, "/quorums")
    ([&](const crow::request& req) {
        return SevaBitBlocks.render_quorum_states_html(true /*add_header_and_footer*/);
    });

    // TODO(SevaBit): This should be combined into the normal search mechanism, we shouldn't have 2 search bars.
    CROW_ROUTE(app, "/search_service_node").methods("GET"_method)
    ([&](const crow::request& req) {
        return SevaBitBlocks.show_service_node(remove_bad_chars(string(req.url_params.get("value"))));
    });
    ...
    ([&](const crow::request& req)
     {
        ...
        return SevaBitBlocks.show_my_outputs(tx_hash, lok_address,
                                         viewkey, raw_tx_data,
                                         domain);
    });
    ...
    ([&](const crow::request& req, string tx_hash,
        string lok_address, string viewkey)
     {
        ...
        return SevaBitBlocks.show_my_outputs(remove_bad_chars(tx_hash),
    ...
    CROW_ROUTE(app, "/prove").methods("POST"_method)
        ([&](const crow::request& req) {
            ...
            return SevaBitBlocks.show_prove(tx_hash,
                                        lok_address,
                                        tx_prv_key,
                                        raw_tx_data,
                                        domain);
    ...
    ([&](const crow::request& req, string tx_hash,
        ...
        return SevaBitBlocks.show_prove(remove_bad_chars(tx_hash),
                                    remove_bad_chars(lok_address),
    ...
    if (enable_pusher)
    {
        ...
        ([&](const crow::request& req) {
            return SevaBitBlocks.show_rawtx();
        });
        ...
        ([&](const crow::request& req) {
            ...
            if (action == "check")
                return SevaBitBlocks.show_checkrawtx(raw_tx_data, action);
            else if (action == "push")
                return SevaBitBlocks.show_pushrawtx(raw_tx_data, action);
    ...
    if (enable_key_image_checker)
    {
        ...
        ([&](const crow::request& req) {
            return SevaBitBlocks.show_rawkeyimgs();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return SevaBitBlocks.show_checkrawkeyimgs(raw_data, viewkey);
        });
    }
    ...
    if (enable_output_key_checker)
    {
        ...
        ([&](const crow::request& req) {
            return SevaBitBlocks.show_rawoutputkeys();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return SevaBitBlocks.show_checkcheckrawoutput(raw_data, viewkey);
        });
    }
    ...
    ([&](const crow::request& req) {
        return SevaBitBlocks.search(remove_bad_chars(string(req.url_params.get("value"))));
    });

    CROW_ROUTE(app, "/mempool")
    ([&](const crow::request& req) {
        return SevaBitBlocks.mempool(true);
    });
...
//    ([&](const crow::request& req) {
//        return SevaBitBlocks.altblocks();
//    });
    ...
    if (enable_js)
    {
        ...
        ([&](const crow::request& req) {
            return SevaBitBlocks.get_js_file("jquery.min.js");
        });

        CROW_ROUTE(app, "/js/crc32.js")
        ([&](const crow::request& req) {
            return SevaBitBlocks.get_js_file("crc32.js");
        });

        CROW_ROUTE(app, "/js/biginteger.js")
        ([&](const crow::request& req) {
            return SevaBitBlocks.get_js_file("biginteger.js");
        });

        CROW_ROUTE(app, "/js/crypto.js")
        ([&](const crow::request& req) {
            return SevaBitBlocks.get_js_file("crypto.js");
        });

        CROW_ROUTE(app, "/js/config.js")
        ([&](const crow::request& req) {
            return SevaBitBlocks.get_js_file("config.js");
        });

        CROW_ROUTE(app, "/js/nacl-fast-cn.js")
        ([&](const crow::request& req) {
            return SevaBitBlocks.get_js_file("nacl-fast-cn.js");
        });

        CROW_ROUTE(app, "/js/base58.js")
        ([&](const crow::request& req) {
            return SevaBitBlocks.get_js_file("base58.js");
        });

        CROW_ROUTE(app, "/js/cn_util.js")
        ([&](const crow::request& req) {
            return SevaBitBlocks.get_js_file("cn_util.js");
        });

        CROW_ROUTE(app, "/js/sha3.js")
        ([&](const crow::request& req) {
            return SevaBitBlocks.get_js_file("sha3.js");
        });
        ...
        ([&](const crow::request& req) {
            ...
            return SevaBitBlocks.get_js_file("all_in_one.js");
        });
    ...
    if (enable_json_api)
    {
        ...
        ([&](const crow::request &req, string tx_hash) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_transaction(remove_bad_chars(tx_hash))};
        ...
        ([&](const crow::request &req, string tx_hash) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_rawtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](const crow::request &req, string block_no_or_hash) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_block(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req, string block_no_or_hash) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_rawblock(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_transactions(
        ...
        ([&](const crow::request &req) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_mempool(
                    remove_bad_chars(page), remove_bad_chars(limit))};
        ...
        ([&](const crow::request &req, string search_value) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_search(remove_bad_chars(search_value))};
        ...
        ([&](const crow::request &req) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_networkinfo()};
        ...
        ([&](const crow::request &req) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_emission()};
        ...
        ([&](const crow::request &req) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_outputs(
        ...
        ([&](const crow::request &req) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_outputsblocks(
        ...
        ([&](const crow::request &req) {
            ...
            mylok::jsonresponse r{SevaBitBlocks.json_version()};
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&]() {
            ...
            return SevaBitBlocks.index2(page_no, refresh_page);
        });
    }
